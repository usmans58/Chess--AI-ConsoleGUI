...
class Board:
    ...
    def __str__(self) -> str:
        return self.wrapStringRep(self.make_board(self.pieces))
    ...
    def isCheckmate(self) -> bool:
        ...
        if len(self.getAllMovesLegal(self.currentSide)) == 0: # if all legal moves are 0 then it is checkmate
            for move in self.getAllMovesUnfiltered(not self.currentSide):
                pieceToTake = move.pieceToCapture 
                if pieceToTake and pieceToTake.stringRep == 'K': #if piece to take is King then it is checkmate
                    return True
        return False

    def isStalemate(self) -> bool:
        #if all legal moves are 0 and no checkmate then it is stalemate
        if len(self.getAllMovesLegal(self.currentSide)) == 0 and not self.isCheckmate():
            return True
        else:
            return False
    ...
    def noMatingMaterial(self) -> bool:
        ...
        if len(self.pieces) == 3 and any(piece.stringRep == 'B' or piece.stringRep == 'N'for piece in self.pieces):
            return True
        else:
            return False
    ...
    def make_board(self, pieces: list[Piece]) -> str:
        DISPLAY_LOOKUP = {
            'R': '♜',
            'N': '♞',
            'B': '♝',
        ...
        for y in range(7, -1, -1):
            for x in range(8):
                ...
                background_colour = (
                    Back.BLACK
                    if (x + y) % 2 == 0
                    else Back.LIGHTBLACK_EX
                )
                pieceRep = background_colour + '  ' + Style.RESET_ALL
                if piece:
                    side = piece.side
                    color = self.whiteColor if side == WHITE else self.blackColor
                    fg_color = Fore.WHITE if color == 'white' else Fore.YELLOW
                    pieceRep = f"{fg_color}{background_colour}{DISPLAY_LOOKUP[piece.stringRep]} {Style.RESET_ALL}"
